import { IntermediatePage, IntermediatePageSerialized } from '@typesCommon/HamsterDocument/IntermediatePage';
...
export class IntermediatePageMap {
	...
	updatePage(page: IntermediatePage) {
		...
		this.sizeMapByPageNumber.set(page.number, { x: page.width, y: page.height });
	}
	// & 从序列化的数据生成类
	static makeBySerializedData(pages: IntermediatePageSerialized[]) {
		...
		const sizeMapByPageNumber: IntermediatePageMap['sizeMapByPageNumber'] = new Map();
		pages.forEach(page => {
			mapById.set(page.id, () => Promise.resolve(new IntermediatePage(page)));
			mapByPageNumber.set(page.number, () => Promise.resolve(new IntermediatePage(page)));
			sizeMapByPageNumber.set(page.number, { x: page.width, y: page.height });
		});
		return new IntermediatePageMap(mapById, mapByPageNumber, sizeMapByPageNumber);
	}
	// & 从列表数据生成类
	static makeByInfoList(infoList: { id: string, pageNumber: number, size: Number2, getData: getPage }[]) {
		const mapById: IntermediatePageMap['mapById'] = new Map();
		const mapByPageNumber: IntermediatePageMap['mapByPageNumber'] = new Map();
		const sizeMapByPageNumber: IntermediatePageMap['sizeMapByPageNumber'] = new Map();
...
export class IntermediateDocument {
	public id: string;
	public title: string;
	public get pages(): Promise<IntermediatePage[]> {
		// 按顺序从 pagesMap 获取pages
		return this.pagesMap.getPages();
	};
	public set pages(pages: IntermediatePage[]) {
		pages.forEach(page => {
			this.pagesMap.updatePage(page);
		})
	}
	private pagesMap: IntermediatePageMap;
	static async serialize(doc: IntermediateDocument): Promise<IntermediateDocumentSerialized> {
	...
	constructor({ pagesMap, id, title }: (IntermediateDocumentSerialized) & { pagesMap: IntermediatePageMap }) {
